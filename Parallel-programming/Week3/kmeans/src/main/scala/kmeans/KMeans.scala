...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    val meanToClusterPoints = points.par.groupBy { thePoint => findClosest(thePoint, means) }

    means.map { meanPoint => {
      val thePointSeq = meanToClusterPoints.getOrElse(meanPoint, GenSeq())

      (meanPoint, thePointSeq)
    }}.toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.par.map(oldMeanPoint => {

      val clusterPoints = classified(oldMeanPoint)

      findAverage(oldMeanPoint, clusterPoints)
    })
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {

    val oldAndNewMeans = oldMeans.zip(newMeans)

    oldAndNewMeans.forall { case (oldMean, newMean) =>
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    if (converged(eta)(means, updatedMeans)) updatedMeans else {
