...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(leftBoundaries: Boundaries, rightBoundaries: Boundaries): Boundaries = {

    val newBoundaries = new Boundaries

    // Minimum of bounds
    newBoundaries.minX = Math.min(leftBoundaries.minX, rightBoundaries.minX)
    newBoundaries.minY = Math.min(leftBoundaries.minY, rightBoundaries.minY)

    // Maximum of bounds
    newBoundaries.maxX = Math.max(leftBoundaries.maxX, rightBoundaries.maxX)
    newBoundaries.maxY = Math.max(leftBoundaries.maxY, rightBoundaries.maxY)
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    ...
    val initialMatrix = new SectorMatrix(boundaries, SECTOR_PRECISION)
    parBodies.aggregate(initialMatrix)(
      (matrix, body) => matrix += body,
      (leftMatrix, rightMatrix) => leftMatrix.combine(rightMatrix))
  }
  ...
  def updateBodies(bodies: Seq[Body], quad: Quad): Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.map { theBody => theBody.updated(quad) }.seq
  }
