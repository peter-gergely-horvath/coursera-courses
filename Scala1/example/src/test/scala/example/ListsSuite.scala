 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of negative and positive numbers") {
    assert(sum(List(-2, -3, 0, 5, 6)) === 6)
  }

  test("sum of repeated numbers") {
    assert(sum(List(-42, 0, 42, 42, -42, 42, 0, 0, -42, 42)) === 42)
  }
  ...
  test("max on an empty list throws NoSuchElementException") {
  ...
  test("max of repeated elements") {
    assert(max(List(1, 1, 2, 2, 3, 3, 4, 4, 42, 42, 42, 123, 123)) === 123)
  }

  test("max of positive and negative elements") {
    assert(max(List(-10, 1, 2, 3, 4, 5, 10)) === 10)
  }
